...
public class TestInteractive : MonoBehaviour
{
    ...
    {
        Debug.Log("[MercuryX2] " + go.name + ":OnPointerEnter");
    }

    private void OnPointerExitImage(GameObject go)
    {
        Debug.Log("[MercuryX2] " + go.name + ":OnPointerExit");
    }

    private void OnPointerClickImage(GameObject go)
    {
        Debug.Log("[MercuryX2] " + go.name + ":OnPointerClick");
    }
    ...
    private void IsInternetUnAvailable(bool IsAvilable)
    {
        ...
        if (IsAvilable)
        {
            Debug.Log("[MercuryX2]:当前网络异常");
        }
        else
        { 
            Debug.Log("[MercuryX2]:当前网络正常");
        }
    }
